...
from CorrelationTables.dataProcessing import dataProcessing
...
with PyCallGraph(output=GraphvizOutput()):
    ...
    # print("Graph Data GraphDataStructure--- %f seconds ---" % (time.time() - start_time))
    ...
    # print("Correlation Table --- %f seconds ---" % (time.time() - start_time))
    ...
    # print cProfile.run('Tab_2_AdjacencyMatrix = GraphVisualization(matrix_filename,correlationTable.RegionName,correlationTable.data)')

    # print cProfile.run('GraphWidget(Tab_2_AdjacencyMatrix,Tab_2_CorrelationTable,correlationTable)')
    ...
    # cProfile.run('widget.setLayout(\'fdp\')',filename = "my.profile")
    ...
    # s.close()
    # Visit stuff Ignore for now FIX ME comment only the stuff 
    ...
    if MainWindowShowFlag:
        main.show()
# print("Startup Time --- %f seconds ---" % (time.time() - start_time5))
